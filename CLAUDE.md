...
- **案内カードを止める理由は 1 か所で決める**（`CarPlayCoordinator.refreshTripPause`）。
  ...
  - **戻すときの理由を取り違えない。** 引き直しから戻るなら経路は入れ替わっているので
    ...
    （`RouteChangeReason.resumed`）。乗っただけで「外れたから引き直した」と車に言わない。
  - **`hasJoinedRoute` は一度立つと下りない。** そこから外れるのは逸脱であって
    「まだ始まっていない」ではないので、走行中に「経路へ進む」が引き直しのカードと
    入れ替わりに出ることはない。
  - 立ち寄り先が増えたときに渡し直すかの判定は、**「止まっているか」ではなく
    「引き直し中か」で見る**（`pauseReason != .rerouting`）。「経路へ進む」で止めている
    あいだに経由地が増えても、渡し直しは要る。
