...
enum ManeuverCard {
    /// 一般道で出すカードの地色。**日本の案内標識と同じ青**（JIS 安全色の青）。
    ///
    /// **`systemBlue` は使わない**（2026-09-12 まではそれだった）。水色寄りで、標識より
    /// 薄く見えるうえ白い文字とのコントラストが 4.0:1 しかない。標識の青は彩度が高く
    /// 輝度が低い——**白抜きで読ませるためにそうなっている**ので、そちらへ寄せる（5.7:1）。
    /// **輝度を上げると青さが薄まって文字も沈む**ので、「明るくする」方向へは動かさない。
    /// `CPMapTemplate.guidanceBackgroundColor` に渡す（`CarPlayCoordinator`）。
    ///
    /// **地図の経路の線は `systemBlue` のまま。** あちらは地図に重ねる線で、標識ではない。
    static let ordinaryColor = UIColor(red: 0x00 / 255, green: 0x67 / 255, blue: 0xC0 / 255, alpha: 1)

    /// 高速の上で出すカードの地色。**日本の案内標識と同じ緑**。
    ///
    /// **`systemGreen` は使わない。** 明るすぎて白い文字が沈むうえ（白とのコントラスト比は
    /// 2.2:1。一般道の青は 5.7:1、この緑は 6.6:1）、「成功」を知らせる色に見える。標識の緑は
