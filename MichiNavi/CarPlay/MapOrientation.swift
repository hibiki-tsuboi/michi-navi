...
enum MapOrientation: String {
    ...
    ///
    /// 進行方向表示は浅く傾け、立体の建物が経路に重なる範囲を抑える。
    /// MapKit は経路表示中に `showsBuildings = false` を無視するため、
    /// 建物を消す設定では解決できない。45 / 30 / 20 / 10 度で描画を比べ、
    /// 建物の張り出しが小さく、経路を追いやすい 10 度にしている。
    ///
    /// これは**追従中に自動で決まる傾き**。2 本指で傾けたぶんはカメラ側に残り、
    /// 現在地ボタンで追従に戻したときにここへ引き戻される。自分で傾けたのなら
    /// 北上げでも寝かせてよく、既定に戻す手段もあるので止めていない。
    var pitch: CGFloat {
        switch self {
        case .heading: 10
        case .north: 0
