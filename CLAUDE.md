...
- **駐車場の提案の実走確認**（2026-08-16 追加ぶん）。ビルドは通っているが、**実際に
  ...
  起こせる（`.parking` の検索は普通の MapKit 検索）。残っているのは 2 つ:
  出るタイミングが手前 1km で早すぎ・遅すぎでないか、そして**「ここに停める」を押した
  あとに案内が駐車場まで引き直されるか**。
  **データ側の心配は済んでいる**。2026-08-16 に MapKit を直接叩いて、タイムズ・リパーク・
  NPC24H・名鉄協商などのコインパーキングが `.parking` で返ることを 14 地点で確認した
  （都心で 24〜38 件／500m、白川郷のような山間の観光地でも 4 件）。代わりに見つかったのが
  駐輪場の混入で、そちらは `ParkingName` で塞いである。
