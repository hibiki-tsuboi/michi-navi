...
- **ルート提示中だけ「この経路について」を出す**（`CPInformationTemplate`、
  ...
  - **営業時間は出せない。** `MKMapItem` が持つのは identifier / location / address / name /
    ...
    何も起きないボタンを作らない）。
- **`CPPointOfInterestTemplate` はナビアプリでは使えない**。地図とカードで検索結果を出せる
  唯一のテンプレートだが、**ガイド p.15 の対応表で Navigation の列だけが空白**
  （Driving task / EV charging・給油・駐車 / Quick food / Public safety にはある）。
  「サポート外のテンプレートを使うと実行時に例外」と同じページに明記されているので、
  push した瞬間に落ちる。2026-08-16 に一度入れて、実機へ持っていく前に対応表で気づいて戻した。
  検索結果は `CPListTemplate` のまま。
  - **対応表は PDF のテキスト抽出では読めない。** 丸印の列位置が落ちるので、
    「Information は 6 個」までしか分からず、どの列が空きかが判別できない。
    **該当ページを画像にして目で見ること**（`PDFPage.draw` で PNG にできる）。
