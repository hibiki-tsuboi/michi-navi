...
**聞かれたときに測って返すだけ**（→「ひと走りの収穫」）。`SolarPosition` ↔ `SunGlareAdvisor`、
`JunctionGeometry` ↔ `JunctionImage` と同じ「測る側／使う側」の分け方で、測る側に置いてある。
...
- **数えるのは出発時からの差**（`Mark`）。`Visit.first` の日付で数えると、日付をまたぐ走行と
  ...
  控え直すとそこまでの収穫が消える）。
- **`PassthroughSubject` を足していない。** 読み上げは `VoiceGuidance` が `arrived` の
  sink の中で `harvest()` を呼んで、到着のひと言の直後に積む。**呼ぶ場所が決まっている**——
  `NavigationController` は `arrived` を流してから `activeRoute` を nil にするので、
  **そこでしか測れない**（控えはその立ち下がりで捨てる）。同じ sink で順に積むことで、
  購読の順番しだいで**収穫が到着より先に鳴る**形にもならない。2 つとも最後まで残るのは
  `isAnnouncingArrival` のおかげ（→「音声案内」）。
...
- **画面は足していない。** 貯まっていくところは `TrackSheet` にもうある（走った距離・
  都道府県・市区町村）。CarPlay にも**画面としては**出さない——`mapButtons` は 4 つで埋まり、
  案内中はナビゲーションバーも左右とも埋まっているので、**足すなら何かを外すことになる**。
  そもそも数を読むのは運転席の仕事ではない（駐車位置を出していないのと同じ）。
  **地図の線だけは別**で、あちらは行もボタンも使わない（→「走った道」）。
